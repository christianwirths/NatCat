...
def prepare_mcmc_data(track_data: list, grid_size: float =2.0) -> pd.DataFrame:
    """
...
def build_transition_dictionary(mcmc_df):
    """
    ...
    """
    print("Building fast transition lookup dictionary...")
    transition_dict = {}
...
class SyntheticTCCatalog:
    ...
    def fit(self, data_path: str):
        ...
        self.mcmc_data = prepare_mcmc_data(self.track_data, grid_size=self.grid_size)
        self.transition_dict = build_transition_dictionary(self.mcmc_data)

        self._is_fitted = True
        print("Model fitted. Ready to generate synthetic storms.")
        return self
    ...
    def generate(self, n_storms: int = None, n_years: int = None) -> pd.DataFrame:
        ...
        if n_years is not None:
            ...
            print(f"Poisson sampling: {n_storms} storms across {n_years} synthetic years.")
        ...
        synthetic_catalog = []
        print(f"Generating tracks for {len(synthetic_origins)} synthetic origins...")
        ...
        print(f"Simulation complete! Generated {len(synthetic_origins)} synthetic storms ({len(catalog)} track points).")
